...
impl Level {
    pub fn read_xml(xml: &str) -> anyhow::Result<Self> {
        ...
        let scene = doc
            .descendants()
            .find(|node| node.tag_name().name().eq_ignore_ascii_case("Scene"))
            .ok_or_else(|| anyhow::anyhow!("xml does not have a <Scene> node"))?;

        let mut model: Option<String> = None;
        let mut triggers: Vec<Trigger> = Vec::new();

        for child in scene.children().filter(|child| child.is_element()) {
            ...
            if child_name.eq_ignore_ascii_case("Trigger") {
                let trigger = Trigger::read(child)?;
